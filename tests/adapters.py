...
def run_swiglu(
    ...
    swilu = Positionwise_fd(d_model, d_ff)
    
    print(f"d_model: {d_model}")
    print(f"d_ff: {d_ff}")
    print(f"in_features: {in_features.shape}")
    swilu.w1.weight.data.copy_(w1_weight)
    swilu.w2.weight.data.copy_(w2_weight)
    swilu.w3.weight.data.copy_(w3_weight)
    output = swilu(in_features)
    print(f"output: {output}")
    print(f"output.shape: {output.shape}")
...
def run_multihead_self_attention(
    ...
    model.out_proj.weight.data.copy_(o_proj_weight)
...
def run_train_bpe(
    ...
    **kwargs,
    ...
    res = train_bpe(input_path, vocab_size, special_tokens)
