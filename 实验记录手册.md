...
**词级分词的问题**：OOV（未登录词）严重，无法处理生僻词、拼写错误；
字符 / 字节级分词的问题：序列过长，增加模型训练计算量，无法捕捉语义单元；
字节级 BPE 的优势：以 256 个字节为基础，无 OOV 问题（任何文本均可转为 UTF-8 字节），通过合并高频字节对形成子词，平衡「语义单元捕捉」和「序列长度压缩」。
