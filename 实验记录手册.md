...
子词分词指的是将一个文本或者句子按照某种规则切分成比“词”更小或更灵活的单位（subword），这些单位可以是字节、字符，也可以是若干字符拼成的子词片段。子词的选取会影响到分词效果。

例子：`"What are some reasons to prefer training our tokenizer on UTF-8 encoded bytes"`

- **词级别**：大致形如 `["What", " ", "are", " ", "some", ..., "bytes"]`（有的实现会把前导空格并到下一个 token 里，比如 `"ĠWhat"`）。
- **字符/字节级别**：形如 `[b"W", b"h", b"a", ..., b"s"]`，其中 `b"_"` 表示字符 `_` 的字节形式。
- **子词 / 字节对级别（BPE）**：在从字节开始逐步合并高频相邻字节对之后，可能得到形如 `[b"Wh", b"at", b" ", b"are", ..., b"bytes"]` 这样的序列，其中有的 token 是单字节，有的是多个字节的组合。

> NOTE:

(1) **词级分词的问题**：`OOV`（未登录词）严重，无法处理生僻词、拼写错误；

(2) **字符 / 字节级分词的问题**：序列过长，增加模型训练计算量，无法捕捉语义单元；

(3)字节级 BPE 的优势：以 256 个字节为基础，无 `OOV` 问题（任何文本均可转为 UTF-8 字节），通过合并高频字节对形成子词，平衡「语义单元捕捉」和「序列长度压缩」。

**BPE 的核心迭代逻辑**：统计高频→合并→更新词汇表，本质是贪心的压缩算法，合并规则的顺序决定了编码的优先级，这也是 BPE 编码具有确定性的原因（相同文本在同一套合并规则下编码结果唯一）。

> **Q:** 为什么会选择使用UTF-8来进行编码？
> **A:** 由于UTF-8是变长编码（多字节编码），1-4字节表示一个字符，且多字节不可拆分，而Unicode的码点过多不利于压缩。

## 2.2 BPE分词流程

### 2.2.1 训练阶段

- **初始化词汇表**： 初始化256个字节的基础词汇表，同时添加特殊令牌（比如 `<|endoftext|>`）。
- **预分词（Pre-tokenization）**: 为了防止跨越单词或标点边界进行不合理的合并（例如将 dog! 中的 g 和 ! 合并），BPE首先使用正则表达式 `PAT = r"""'(?:[sdmt]|ll|ve|re)| ?\p{L}+| ?\p{N}+| ?[^\s\p{L}\p{N}]+|\s+(?!\S)|\s+"""`对语料预分词，将原始文本切分为“预Token”（pre-tokens），每个预Token被视为独立的单元，随后的合并操作不会跨越这些单元的边界。
    ```python 

    def pre_tokenizer(text:str , special_tokens: list[str],drop_special_token: bool = True) -> list[str]:
        """
        对文本进行预分词，将文本切分为预Token序列。

        参数:
        - text (str): 输入的原始文本。
        - special_tokens (list[str]): 特殊令牌列表，用于在预分词中特殊处理。
        - drop_special_token (bool, 可选): 是否在预分词结果中删除特殊令牌。默认值为 True。

        返回:
        - list[str]: 预分词后的预Token序列。
        """
...
- **BPE 合并规则计算**：
  ◦ 统计所有预Token中相邻字节对的出现频率。

  ```python
          # 
         for j , pretoken in enumerate(pretokens):
              # pretoken : b'hello'
              # pretokens  :  [b'Hello',b'my',b'friends']
  ...
  ◦ 找到频率最高的字节对（例如字节 e 和 s），将其合并为一个新Token `es`，并加入词汇表。

  ```python
      for i in range(num_merges):  
          max_pair = max(
              # 待排序的字典项，形式为 ((index1, index2), count)
              counts.items(),  
              # 排序键的规则
              key=lambda x: (   
                # x:((idx1,idx2),freq),x[1] : freq ,x[0]:(idx1,idx2),x[0][0]:idx1 , x[0][1]:idx2
                  x[1],  # 频率（主要排序依据）
  ...
  ◦ 如果出现频率相同，通常选择字典序更大的对进行合并（作为打破平局的规则）。
    ```python
        # 词汇表（简化）
        vocab = {
            0: b'a',
    ...
    - 排序过程
        ...
        结果：选择 (0, 1)（即 'a' 和 'b'）进行合并
  ◦ 合并操作：
    ```python
        def merge(counts, index_dict, pretokens, max_pair, new_index):
            ...
            new_index: 合并后新标记的索引
    ```
    - (1) 获取需要处理的序列（包含需要合并的tokens）。
    - (2) 每个序列都执行合并，更新统计信息，处理和前一个字符和后一个字符的关系。
    ```text
    原始: [1, 2, 3, 1, 2, 4]
    扫描到 (1,2) → 替换为 6
    结果: [6, 3, 6, 4]
    ...
    * 后字符是4:
        ...
        - 增加 (6,4) 计数: 0 → 1
    ```
  ◦ 重复此过程，直到词汇表达到预设的大小（例如32,000或50,000）
- **特殊令牌处理：** 预分词前剥离语料中的特殊令牌，避免跨文档边界合并。



### 2.2.2 使用TinyStorie数据集完成训练BPE分词器

- 使用cProfile/scalene性能分析工具
- **scalene** : 高性能分析器
- 基本特性 
    - 三合一分析： CPU,GPU,内存
    - 低开销： 使用采样技术，开销低
...
- 1. 预分词： 对输入文本进行同样的正则预分词；
- 2. 转换字节： 将每个预Token转换为UTF-8字节序列；
- 3. 应用合并： 按照训练阶段生成的合并规则列表（按创建顺序），依次将字节序列中的对合并为对应的Token ID。
- 4. 最终输出一个整数ID序列。
...
- 1. 查表：根据ID在词汇表中查找对应的字节序列。
- 2. 拼接与解码：将所有字节序列拼接，然后使用UTF-8格式解码为Unicode字符串,。
- 3. 错误处理：如果拼接后的字节序列包含无效的UTF-8序列，通常会用Unicode替换字符 U+FFFD（）进行替换
